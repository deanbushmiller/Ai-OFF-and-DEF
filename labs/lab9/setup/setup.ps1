# ===========================================================================
#  SecLLM Bootcamp - Lab 9 setup  (Windows)
#
...
$Lab       = 'lab9'
$NextLab   = 'lab10'         # set to '' on the final lab
$NextName  = 'Lab 10 - Defending RAG ingestion'
$Container = 'seclm-lab9-run'
$Here      = Split-Path -Parent $MyInvocation.MyCommand.Path
$Results   = Join-Path $Here 'lab9-results.txt'
...
Write-Host ""
Hr
Write-Host "  SecLLM Bootcamp - Lab 9: Defending the model supply chain"
...
if ($alreadyHave) {
    ...
    Write-Host "  Downloading the lab image. About 249 MB on Intel, a couple"
    Write-Host "  of minutes on a normal connection. Measured on the published"
    Write-Host "  image 2026-09-19."
    Write-Host ""
    Write-Host "  This lab uses the same model and libraries as lab 1, so it"
    Write-Host "  LOOKS like it should be a small download. It is not, yet:"
    Write-Host "  lab 9 was built on a newer base image than labs 1-8 were, and"
    Write-Host "  Docker only reuses a layer when it matches exactly. That is a"
    Write-Host "  rebuild job on our side, not something wrong on yours."
    Write-Host "  $Tag"
    Hr
    Write-Host ""
    & docker pull $Tag
...
Write-Host "  That is not caution, it is the subject: you are about to load"
Write-Host "  files that try to phone home, and the first control that stops"
Write-Host "  them is the one you can see on the command line."
Write-Host ""
Write-Host "  Starting the lab. You will be asked to choose beginner or"
Write-Host "  expert mode. Beginner types 9 checked commands; expert gets"
Write-Host "  a real shell and works from LAB.md."
Write-Host ""
Write-Host "  Nothing here runs a language model, so nothing is slow. Each"
Write-Host "  command returns in a second or two."
Hr
Write-Host ""

# --network none, and this is a DELIBERATE break from labs 1-8.
#
# Every earlier lab left the flag off on the grounds that forcing "none" would
# only confuse a student about to be offered the next lab's download. That
# reasoning does not survive contact with this lab: lab 9 teaches egress
# control, one of its artifacts tries to open a socket, and the lab's own
# output tells the student to check that the container has no route out.
#
...
& docker run -it --network none --name $Container $Tag lab 9 @ExtraArgs
$runRc = $LASTEXITCODE
...
if ($NextLab) {
    ...
    if ($getNext -match '^[Nn]') {
        ...
        Info " That is expected and harmless - lab 9 is already complete.)"
        Write-Host ""
        & docker pull $NextTag
        if ($LASTEXITCODE -eq 0) {
            Write-Host ""
...
Write-Host ""
Hr
Write-Host "  Lab 9 complete. The image stays on your machine for the next lab."
Hr
Write-Host ""
Read-Host "Press Enter to close"
