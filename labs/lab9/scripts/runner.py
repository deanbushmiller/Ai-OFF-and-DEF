...
def fail(msg):
    ...
    say(" Send lab9-results.txt to the instructor via email or post to class Q&A.")
    rule()
    sys.exit(1)
...
def expert_shell():
    ...
    say("    - gate the beacon file BEFORE you sandbox and quarantine it:")
    say("        python gate.py bert_tiny_beacon.pt")
    say("      quarantine.py moves the file, so this only works while it is")
    say("      still there. Read which line blocks it - the scanner's, not yours.")
