# ===========================================================================
#  SecLLM Bootcamp - Lab 8 setup  (Windows)
#
...
$Lab       = 'lab8'
$NextLab   = ''              # EMPTY: lab 8 is the final lab of part 1
$NextName  = ''
$Container = 'seclm-lab8-run'
$Here      = Split-Path -Parent $MyInvocation.MyCommand.Path
$Results   = Join-Path $Here 'lab8-results.txt'
...
Write-Host ""
Hr
Write-Host "  SecLLM Bootcamp - Lab 8: Offensive recap and transition to defense"
...
# No -p, and nothing to publish: lab 8 serves nothing at all. No model, no
# listener, no port. 8008 was reserved for a worksheet UI and is deliberately
# left unclaimed.
& docker run -it --name $Container $Tag lab 8 @ExtraArgs
$runRc = $LASTEXITCODE
...
& docker cp "$Container`:/labs/lab8/atlas-layer.json" $Layer *> $null
if (Test-Path $Layer) {
...
# Lab 8 is the final lab of part 1, so $NextLab is empty and the pre-pull block
# that labs 1-7 carry has been DELETED rather than guarded. Dead code with stale
# text in it is how labs 5 and 6 came to greet Windows students with lab 4's
...
Write-Host "    lab 3  the hijacked summary"
Write-Host "    lab 4  the OCR injection and the flipped classifier"
...
Write-Host ""
Hr
Write-Host "  Lab 8 complete. That is the end of part 1."
Hr
Write-Host ""
Read-Host "Press Enter to close"
