...
if docker image inspect "$TAG" >/dev/null 2>&1; then
  ...
  printf '\n'
  printf '  If you have done any of labs 3 to 7, 12, 13 or 14 on this\n'
...
printf '\n'
printf '  If you did labs 3 to 7 or 12 to 14 on this machine, the 1.09 GB\n'
...
docker run -it --network none --name "$CONTAINER" "$TAG" lab 15 ${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}
RUN_RC=$?
