#!/usr/bin/env bash
# ===========================================================================
#  SecLLM Bootcamp - Lab 7 setup  (macOS and Linux)
#
...
LAB="lab7"
NEXT_LAB="lab8"          # set to "" on the final lab
NEXT_LAB_NAME="Lab 8 - Offensive recap and transition to defense"
CONTAINER="seclm-lab7-run"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS="$HERE/lab7-results.txt"
EXTRA_ARGS=("$@")
...
printf '\n'; hr
printf '  SecLLM Bootcamp - Lab 7: AI-powered attack orchestration\n'
...
printf '  The model step takes roughly a minute, and up to five on a slow\n'
printf '  machine. That is the lab working, not the lab hanging.\n'
hr; printf '\n'
...
# Publishing a port here would add a Windows failure mode and buy the student
# nothing. No --network flag either - the lab needs no network, but forcing
# "none" here would only confuse a student who is about to be offered the next
# lab's download.
#
# NOTE for the 8-lab consolidation: 8007 is also lab 6's INTERNAL port (its real
...
printf '\n'
if docker cp "$CONTAINER:/labs/lab7/lab7-results.txt" "$RESULTS" >/dev/null 2>&1; then
...
printf '\n'; hr
printf '  Lab 7 complete. The image stays on your machine for the next lab.\n'
hr; printf '\n'
