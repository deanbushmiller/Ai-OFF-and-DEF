# ===========================================================================
#  SecLLM Bootcamp - Lab 7 setup  (Windows)
#
...
$Lab       = 'lab7'
$NextLab   = 'lab8'          # set to '' on the final lab
$NextName  = 'Lab 8 - Offensive recap and transition to defense'
$Container = 'seclm-lab7-run'
$Here      = Split-Path -Parent $MyInvocation.MyCommand.Path
$Results   = Join-Path $Here 'lab7-results.txt'
...
Write-Host ""
Hr
Write-Host "  SecLLM Bootcamp - Lab 7: AI-powered attack orchestration"
...
if ($alreadyHave) {
    ...
    Write-Host "  Downloading the lab image. About 1.2 GB, usually 3-8 minutes."
...
Write-Host ""
Hr
Write-Host "  This lab serves nothing and needs no network once the image is"
Write-Host "  here. Everything happens inside the container."
Write-Host ""
...
Write-Host ""
Write-Host "  Four of the seven commands run a language model on your CPU."
Write-Host "  The model step takes roughly a minute, and up to five on a slow"
Write-Host "  machine. That is the lab working, not the lab hanging."
Hr
Write-Host ""

# No -p on purpose. Lab 7 DOES serve - a mock collector on 127.0.0.1:8007 -
# but it is inside the container and nothing outside needs to reach it.
# Publishing a port here would add a Windows failure mode and buy nothing.
#
# NOTE for the 8-lab consolidation: 8007 is also lab 6's INTERNAL port. Harmless
# - neither is published and the two labs never run at once - but it will look
# like a conflict when the images merge. Recorded in both labs' assets.json.
& docker run -it --name $Container $Tag lab 7 @ExtraArgs
$runRc = $LASTEXITCODE

# --- 9. Recover the transcript ----------------------------------------------
Write-Host ""
& docker cp "$Container`:/labs/lab7/lab7-results.txt" $Results *> $null
...
Write-Host ""
Hr
Write-Host "  Lab 7 complete. The image stays on your machine for the next lab."
Hr
Write-Host ""
Read-Host "Press Enter to close"
