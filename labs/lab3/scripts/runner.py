...
def fail(msg):
    ...
    say(" Send lab3-results.txt to the instructor via email or post to class Q&A.")
    rule()
    sys.exit(1)
...
def expert_shell():
    ...
    say("")
    say("  The mock site is already running on 127.0.0.1:8003, INSIDE this")
    say("  container. It is not published to your machine and cannot be, so")
    say("  read the pages with curl from in here:")
    say("      curl -s http://news.acme.com:8003/article-poisoned.html | tail -25")
    say("  That is also the only view that shows you the payload - a browser")
    say("  is built to hide exactly the block this lab is about.")
    say("")
