...
SHARED by labs 3, 4, 5, 6 and 7 - the whole model tier. Keep this exact repo ID,
filename and destination so the merged 8-lab image stores it once.

This file is the SAME in every lab of the tier, deliberately: the COPY layer it
produces is only shared if it is byte-identical, so the line above is written to
be true from any lab's point of view rather than from one lab's.
