#!/usr/bin/env bash
# ===========================================================================
#  SecLLM Bootcamp - Lab 6 setup  (macOS and Linux)
#
...
LAB="lab6"
NEXT_LAB="lab7"          # set to "" on the final lab
NEXT_LAB_NAME="Lab 7 - AI-powered attack orchestration"
CONTAINER="seclm-lab6-run"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS="$HERE/lab6-results.txt"
EXTRA_ARGS=("$@")
...
printf '\n'; hr
printf '  SecLLM Bootcamp - Lab 6: MCP and interface hijacking\n'
...
# No -p on purpose. Lab 6 DOES serve - three processes on 127.0.0.1:8006
# and :8007 - but all of them are inside the container and none of them
# needs to be reachable from the host. Publishing a port here would add a
...
printf '\n'
if docker cp "$CONTAINER:/labs/lab6/lab6-results.txt" "$RESULTS" >/dev/null 2>&1; then
...
printf '\n'; hr
printf '  Lab 6 complete. The image stays on your machine for the next lab.\n'
hr; printf '\n'
