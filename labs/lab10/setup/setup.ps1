# ===========================================================================
#  SecLLM Bootcamp - Lab 10 setup  (Windows)
#
...
$Lab       = 'lab10'
$NextLab   = 'lab11'         # set to '' on the final lab
$NextName  = 'Lab 11 - Defending multimodal input'
$Container = 'seclm-lab10-run'
$Here      = Split-Path -Parent $MyInvocation.MyCommand.Path
$Results   = Join-Path $Here 'lab10-results.txt'
...
Write-Host ""
Hr
Write-Host "  SecLLM Bootcamp - Lab 10: Defending RAG ingestion"
...
if ($alreadyHave) {
    ...
    Write-Host ""
    Write-Host "  If you have done lab 2 on this machine, this is about 34 KB"
    Write-Host "  and takes a second: lab 10 reuses lab 2's models and libraries"
    Write-Host "  down to the byte. From a clean machine it is about 628 MB on"
...
# reasoning does not survive contact with this lab: lab 10 teaches ingestion
...
#
# No -p: lab 10 serves nothing. The contract reserves 8010 and it stays unclaimed.
& docker run -it --network none --name $Container $Tag lab 10 @ExtraArgs
$runRc = $LASTEXITCODE
...
if ($NextLab) {
    ...
    Info "about 24 MB on Intel; from a clean machine about 105 MB. Measured"
    Info "on the published image. You are not paying twice for anything."
    Write-Host ""
    ...
    if ($getNext -match '^[Nn]') {
        ...
        Info " That is expected and harmless - lab 10 is already complete.)"
        Write-Host ""
        & docker pull $NextTag
        if ($LASTEXITCODE -eq 0) {
            Write-Host ""
...
Write-Host ""
Hr
Write-Host "  Lab 10 complete. The image stays on your machine for the next lab."
Hr
Write-Host ""
Read-Host "Press Enter to close"
