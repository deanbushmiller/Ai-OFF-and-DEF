#!/usr/bin/env bash
# ===========================================================================
#  SecLLM Bootcamp - Lab 10 setup  (macOS and Linux)
#
...
LAB="lab10"
NEXT_LAB="lab11"         # set to "" on the final lab
NEXT_LAB_NAME="Lab 11 - Defending multimodal input"
CONTAINER="seclm-lab10-run"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS="$HERE/lab10-results.txt"
EXTRA_ARGS=("$@")
...
printf '\n'; hr
printf '  SecLLM Bootcamp - Lab 10: Defending RAG ingestion\n'
...
printf '  That is not caution, it is the subject: you are about to load\n'
printf '  files that try to phone home, and the first control that stops\n'
printf '  them is the one you can see on the command line.\n'
printf '\n'
printf '  Starting the lab. You will be asked to choose beginner or\n'
printf '  expert mode. Beginner types 9 checked commands; expert gets\n'
printf '  a real shell and works from LAB.md.\n'
printf '\n'
printf '  Nothing here runs a language model, so nothing is slow. Each\n'
...
# only confuse a student who is about to be offered the next lab's download.
# That reasoning does not survive contact with this lab: lab 10 teaches ingestion
# control, one of its artifacts tries to open a socket, and the lab's own
# output tells the student to check that the container has no route out. A lab
# that says "isolate it" while running unisolated is teaching the wrong lesson
# twice.
#
...
docker run -it --network none --name "$CONTAINER" "$TAG" lab 10 ${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}
RUN_RC=$?
...
printf '\n'; hr
printf '  Lab 10 complete. The image stays on your machine for the next lab.\n'
hr; printf '\n'
