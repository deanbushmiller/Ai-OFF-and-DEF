#!/usr/bin/env bash
# ===========================================================================
#  SecLLM Bootcamp - Lab 5 setup  (macOS and Linux)
#
...
LAB="lab5"
NEXT_LAB="lab6"          # set to "" on the final lab
NEXT_LAB_NAME="Lab 6 - MCP and interface hijacking"
CONTAINER="seclm-lab5-run"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS="$HERE/lab5-results.txt"
EXTRA_ARGS=("$@")
...
printf '\n'; hr
printf '  SecLLM Bootcamp - Lab 5: Exploiting AI agents and excessive agency\n'
...
printf '  expert mode. Beginner types 6 checked commands; expert gets\n'
printf '  a real shell and works from LAB.md.\n'
printf '\n'
printf '  Three of the six commands run a language model on your CPU.\n'
printf '  Each takes roughly a minute, and up to two on a slow machine.\n'
printf '  That is the lab working, not the lab hanging.\n'
hr; printf '\n'

# No -p: lab 5 does not serve. No --network flag either - the lab needs no
# network, but forcing "none" here would only confuse a student who is about
# to be offered the next lab's download.
docker run -it --name "$CONTAINER" "$TAG" lab 5 ${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}
RUN_RC=$?

# --- 8. Recover the transcript ----------------------------------------------
printf '\n'
if docker cp "$CONTAINER:/labs/lab5/lab5-results.txt" "$RESULTS" >/dev/null 2>&1; then
  ok "Results saved: $RESULTS"
  info "Paste all three tool-call chains from the evidence block into the chat."
else
...
if [ -n "${NEXT_LAB:-}" ]; then
  ...
  printf '\n'
  info "$NEXT_LAB_NAME"
  info "Lab 6 reuses the same language model, and the two images SHARE it."
  info "Measured on the published images: about 30 KB, not another 1.2 GB."
  info "This should take seconds, not minutes."
  printf '\n'
  ...
  case "${GETNEXT:-y}" in
    ...
    *)
      ...
      info " That is expected and harmless - lab 5 is already complete.)"
      printf '\n'
...
printf '\n'; hr
printf '  Lab 5 complete. The image stays on your machine for the next lab.\n'
hr; printf '\n'
