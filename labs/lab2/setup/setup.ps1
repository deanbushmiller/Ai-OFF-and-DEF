# ===========================================================================
#  SecLLM Bootcamp - Lab 2 setup  (Windows)
#
...
$Lab       = 'lab2'
$NextLab   = 'lab3'          # set to '' on the final lab
$NextName  = 'Lab 3 - Advanced prompt injection'
$Container = 'seclm-lab2-run'
$Here      = Split-Path -Parent $MyInvocation.MyCommand.Path
$Results   = Join-Path $Here 'lab2-results.txt'
...
Write-Host ""
Hr
Write-Host "  SecLLM Bootcamp - Lab 2: RAG and semantic ingestion attacks"
...
& docker run -it --name $Container $Tag lab 2 @ExtraArgs
$runRc = $LASTEXITCODE

# --- 9. Recover the transcript ----------------------------------------------
Write-Host ""
& docker cp "$Container`:/labs/lab2/lab2-results.txt" $Results *> $null
...
if ($NextLab) {
    ...
    if ($getNext -match '^[Nn]') {
        ...
        Info " That is expected and harmless - lab 2 is already complete.)"
        Write-Host ""
        & docker pull $NextTag
        if ($LASTEXITCODE -eq 0) {
            Write-Host ""
...
Write-Host ""
Hr
Write-Host "  Lab 2 complete. The image stays on your machine for the next lab."
Hr
Write-Host ""
Read-Host "Press Enter to close"
